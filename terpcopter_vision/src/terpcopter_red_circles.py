...
from terpcopter_comm.msg import DetectTarget
...
class image_converter:
  ...
  def __init__(self):
    ...
    #self.redtarget_pos = rospy.Publisher("/redTargetPose", DetectTarget, queue_size=10 )
    self.redtarget_pos = rospy.Publisher("/redTargetPose", PoseStamped, queue_size=10 )

    self.bridge = CvBridge()
    self.image_sub = rospy.Subscriber("/iris/camera_iris/image_raw",Image,self.callback)
  ...
  def callback(self,ros_data):
    ...
    for contour in cnts:
      ...
      if (width < 500) and (height < 500) and (width >= 0) and (height > 0) and (area>1000):
        ...
        # yaw = math.atan2(Rt[0][1], Rt[0][0])* (180/math.pi)
        y_pos= np.array(translation_vector[0],dtype=float) #cm
        x_pos= np.array(translation_vector[1],dtype=float) #cm
        ...
        cv2.putText(cv_image, "X: %.2fcm" % x_pos, (450,  300), #swapped x and y to make it work
              cv2.FONT_HERSHEY_SIMPLEX,
              1.0, (0, 255, 0), 3)

        cv2.putText(cv_image, "Y: %.2fcm" % y_pos, (150,  500),
              cv2.FONT_HERSHEY_SIMPLEX,
              1.0, (0, 255, 0), 3)
